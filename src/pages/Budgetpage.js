...
function BudgetPage() {
    ...
    const toggleBudgets = () => {
        if (!showBudgets) {
            axios.get(`http://192.168.0.83:8081/budgets?userId=${userId}`)
                .then(res => {
    ...
    const saveBudgetChanges = (id, budget) => {
        axios.put(`http://192.168.0.83:8081/budgets/${id}`, {
            month: budget.month,
    ...
    const handleDelete = (id) => {
        axios.delete(`http://192.168.0.83:8081/budgets/${id}`)
            .then(() => {
                alert("삭제 완료");
                toggleBudgets();
    ...
    const addBudget = () => {
        axios.post(`http://192.168.0.83:8081/budgets`, {
            ...newBudget,
            userId
        }).then(() => {
            alert("예산 추가 완료");
                        ...
                        <tbody>
                            {budgets.map((budget, idx) => (
                                <tr key={budget.id}>
                                    <td>
                                        {isEditMode ?
                                            <input value={budget.month} onChange={e => handleBudgetChange(idx, 'month', e.target.value)} />
                                            : budget.month}
                                    </td>
                                    <td>
                                        {isEditMode ?
                                            <input type="number" value={budget.amount} onChange={e => handleBudgetChange(idx, 'amount', e.target.value)} />
